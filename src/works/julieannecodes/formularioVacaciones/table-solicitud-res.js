...
class TableSolicitudRes extends LitElement {
  static get styles() {
  ...
  tableL() {
    ...
    <div class="tableDiv">
      <div>
        ...
        <div id="${i}">
            <details>
                <summary>from: ${dateFormatter(item.startDate).tableDate} to: ${dateFormatter(item.endDate).tableDate}</summary>
                <div class="contents">
                    <div>${this.tableTitles[0]}: ${dateFormatter(item.currentDate).solicitudDate}</div>
                    <div>${this.tableTitles[1]}: ${dateFormatter(item.startDate).tableDate}</div>
                    <div>${this.tableTitles[2]}: ${dateFormatter(item.endDate).tableDate}</div>
                    <div>${this.tableTitles[3]}: ${item.status}</div>
                    <div>${this.tableTitles[4]}: ${item.statusDate}</div>
                </div>
                <div>
...
window.customElements.define('table-solicitud-res', TableSolicitudRes);
